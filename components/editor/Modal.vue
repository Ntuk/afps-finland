<template>
  <div class="modal" v-if="show">
    <div class="modal-content">
      <h1>Lisää oppaaseen video:</h1>
      <label for="url">Pastea tähän Youtube-videon URL:</label>
      <input v-model="url" id="url" />
      <footer class="modal-footer">
        <button @click="insertVideo">Lisää video</button>
        <button @click="show = false;">Sulje ikkuna</button>
      </footer>
    </div>
  </div>
</template>
